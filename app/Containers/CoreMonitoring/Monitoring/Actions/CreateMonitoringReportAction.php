...
class CreateMonitoringReportAction extends ParentAction
{
    ...
    public function run(Request $request): MonitoringReport
    {
        ...
        $monitoring_items = app(GetOpenMonitoringItemsByElectionScopeTask::class)->run(
            $election->id,
            $scope_type,
            $scope_department,
            $user
        ...
        $data = [
            'code' => substr(
                hash(
                    "sha512",
                    Carbon::now()->timestamp . $user->id .  $election->id . Str::random(24)
                ), 0, 30
            ),
        ...
        return DB::transaction(function () use ($data, $monitoring_items, $request) {
            ...
            $monitoring_report->code = 'R-' . strtoupper(substr(hash(
                "sha512",
                $monitoring_report->id . $monitoring_report->code), 0, 6
                )) . '/' . Carbon::now()->format('y');
