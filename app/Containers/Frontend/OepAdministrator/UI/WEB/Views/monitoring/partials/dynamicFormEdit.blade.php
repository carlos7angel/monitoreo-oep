            ...
            @if(in_array($field->fieldType->type, ['textbox', 'textarea', 'select', 'checkbox', 'datepicker', 'fileupload']))
                <div class="fv-row mb-8">
                    ...
                    @switch($field->fieldType->type)
                        ...
                        @case('textbox')
                            <input type="text" name="{{ $field->unique_fieldname }}" class="form-control mb-2" placeholder="{{ $field->placeholder ? $field->placeholder : '' }}"
                            ...
                            value="{{ $input[$field->unique_fieldname] ? $input[$field->unique_fieldname] : '' }}"/>
                        @break
                        ...
                        @case('textarea')
                            ...
                            {!! ($field->maxlength) ? 'data-fv-string-length___max="'.$field->maxlength.'"' : '' !!}
                            {!! ($field->minlength || $field->maxlength) ? 'data-fv-string-length___message="'. (($field->minlength && $field->maxlength) ? 'La longitud debe estar entre ' . $field->minlength . ' y ' . $field->maxlength : ($field->minlength ? 'Longitud mínima ' . $field->minlength : 'Longitud máxima ' . $field->maxlength) ) .'"' : '' !!}
                            >{{ $input[$field->unique_fieldname] ? $input[$field->unique_fieldname] : '' }}</textarea>
                        @break
                            ...
                            <select class="form-select mb-2" data-control="select2" data-hide-search="{{ $field->select_search ? 'false' : 'true' }}" data-placeholder="{{ $field->placeholder ? $field->placeholder : 'Selecciona una opción' }}"
                                ...
                                @if($field->options_type === 'custom')
                                    ...
                                    @foreach($options as $index => $option)
                                        <option value="{{ $option->value }}" {{ in_array($option->value, $selected) ? 'selected' : '' }}>{{ $option->text }}</option>
                                    @endforeach
                                @endif
                            ...
                            @if($field->options_type === 'catalog')
                                ...
                                @foreach($catalog->items as $index => $option)
                                    <div class="form-check form-check-custom mb-3">
                                        ...
                                        {!! $field->required ? 'data-fv-not-empty="true" data-fv-not-empty___message="El campo es obligatorio"' : '' !!}
                                        {{ in_array($option->id, $selected) ? 'checked="checked"' : '' }} />
                        ...
                        @case('datepicker')
                            <input type="text" name="{{ $field->unique_fieldname }}" class="form-control mb-2 {{ $field->field_subtype === 'time' ? 'kt_form_field_time' : 'kt_form_field_date' }}" placeholder="{{ $field->placeholder ? $field->placeholder : '' }}"
                            {!! $field->required ? 'data-fv-not-empty="true" data-fv-not-empty___message="El campo es obligatorio"' : '' !!}
                            value="{{ $input[$field->unique_fieldname] ? $input[$field->unique_fieldname] : '' }}"/>
                        @break
                        ...
                        @case('fileupload')
                            ...
                            @if(isset($input[$field->unique_fieldname]) && is_array($input[$field->unique_fieldname]))
                                <input type="hidden" name="default_file_{{ $field->unique_fieldname }}" data-name="{{ $input[$field->unique_fieldname]['original_name'] }}" data-size="{{ $input[$field->unique_fieldname]['size'] }}"
                            ...
                            <input type="file" name="{{ $field->unique_fieldname }}" data-default="default_file_{{ $field->unique_fieldname }}" class="files kt_form_field_fileupload" data-maxsize="{{ $field->file_maxsize ? $field->file_maxsize : '3' }}" data-maxfiles="{{ $field->maxfiles ? $field->maxfiles : '1' }}" data-accept="{{ $mimes }}"
                            {!! (isset($input[$field->unique_fieldname]) && is_array($input[$field->unique_fieldname])) ? '' : ($field->required ? 'data-fv-not-empty="true" data-fv-not-empty___message="El campo es obligatorio"' : '') !!}>
