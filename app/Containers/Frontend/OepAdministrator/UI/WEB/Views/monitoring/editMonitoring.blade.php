...
@section('content')
    <div class="content flex-row-fluid" id="kt_content">
        <form id="kt_edit_monitoring_form" class="form d-flex flex-column flex-lg-row" method="post" autocomplete="off"
              action="{{ route('oep_admin_media_monitoring_update', ['election_id' => $election->id, 'id' => $monitoring->id]) }}">
                        ...
                        <div class="mx-auto w-100 mw-600px pt-10 pb-10">
                            ...
                            <div class="mb-10 fv-row">
                                ...
                                <select class="form-select mb-2" data-control="select2" data-hide-search="true" data-placeholder="" name="oep_media_type" readonly="readonly" disabled>
                                    <option value="TV" {{ $monitoring->media_type === 'TV' ? 'selected="selected"' : '' }}>MEDIO TELEVISIVO</option>
                                    <option value="RADIO" {{ $monitoring->media_type === 'RADIO' ? 'selected="selected"' : '' }}>MEDIO RADIAL</option>
                                    <option value="PRINT" {{ $monitoring->media_type === 'PRINT' ? 'selected="selected"' : '' }}>MEDIO IMPRESO</option>
                                    <option value="DIGITAL" {{ $monitoring->media_type === 'DIGITAL' ? 'selected="selected"' : '' }}>MEDIO DIGITAL</option>
                                    <option value="RRSS" {{ $monitoring->media_type === 'RRSS' ? 'selected="selected"' : '' }}>REDES SOCIALES</option>
                                </select>
                            </div>
                            <div class="mb-10 fv-row">
                                ...
                                <div class="d-flex __fv-row">
                                    <div class="form-check form-check-custom form-check-solid">
                                        <input class="form-check-input me-3" name="media_registered" type="radio" value="1" id="kt_type_option_0" {{ $monitoring->registered_media ? 'checked="checked"' : '' }}
                                ...
                                <div class="d-flex __fv-row">
                                    <div class="form-check form-check-custom form-check-solid">
                                        <input class="form-check-input me-3" name="media_registered" type="radio" value="0" id="kt_type_option_1" {{ $monitoring->registered_media ? '' : 'checked="checked"' }}
                                               data-fv-not-empty="true" data-fv-not-empty___message="El campo es obligatorio"/>
                            ...
                            <div class="kt_select_media_input {{ $monitoring->registered_media ? 'd-block' : 'd-none' }}">
                                <div class="mb-10 fv-row">
                                    ...
                                    <select class="form-select mb-2" data-control="select2" data-hide-search="false" data-placeholder="Seleccione un medio" name="media_profile"
                                            data-fv-not-empty="{{ $monitoring->registered_media ? 'true' : 'false' }}" data-fv-not-empty___message="El campo es obligatorio">
                                        <option value=""></option>
                                        @foreach($medias as $media)
                                            <option value="{{ $media->id }}" {{ $monitoring->fid_media_profile == $media->id ? 'selected' : '' }}>{{ $media->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="kt_text_media_input {{ $monitoring->registered_media ? 'd-none' : 'd-block' }}">
                                <div class="mb-10 fv-row">
                                    <label class="required form-label">Medio de Comunicación:</label>
                                    <input type="text" class="form-control" placeholder="" name="media_profile_text" value="{{ $monitoring->other_media }}"
                                           data-fv-not-empty="{{ $monitoring->registered_media ? 'false' : 'true' }}" data-fv-not-empty___message="El campo es obligatorio"/>
                                </div>
                            </div>

                            @include('frontend@oepAdministrator::monitoring.partials.dynamicFormEdit')
...
@section('scripts')
    ...
    <script src="{{ asset('themes/admin/js/custom/monitoring/edit.js') }}"></script>
@endsection